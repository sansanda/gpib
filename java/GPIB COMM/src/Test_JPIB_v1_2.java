...
public class Test_JPIB_v1_2 {
	...
	private static boolean loadLibrary() throws IllegalStateException {
		...
		if (!libraryLoaded) {
			...
			case WINDOWS32:
				System.load("E:\\repos\\java_development_libraries\\comms\\parallel\\GPIB\\JPIB V1.2\\jpib_32.dll");
				break;
				
			case WINDOWS64:
				System.load("E:\\repos\\java_development_libraries\\comms\\parallel\\GPIB\\JPIB V1.2\\jpib_64.dll");
				break;

			default:
				throw new IllegalStateException("No supported driver for OS "+ OSUtils.getOS());
			}
        ...
        GPIBDevice myDevice;
			...
			try {
				...
				while (gpibDevicesList.hasMoreElements())
			    {
					...
					myDevice.writeCommand("DISPLAY:WINDOW1:TEXT:DATA 'HOLA MUNDO'");
